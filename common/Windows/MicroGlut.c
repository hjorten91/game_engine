...
// 210424: Added #define GLUT_WINDOW_WIDTH, GLUT_WINDOW_HEIGHT, GLUT_MOUSE_POSITION_X, GLUT_MOUSE_POSITION_Y and GLUT_QUIT_FLAG for glutGet.
...
static void (*gReshape)(int width, int height);
static void (*gKey)(unsigned char key, int x, int y);
static void (*gKeyUp)(unsigned char key, int x, int y);
static void (*gSpecialKey)(unsigned char key, int x, int y); // I consider this obsolete!
static void (*gSpecialKeyUp)(unsigned char key, int x, int y); // I consider this obsolete!
static void (*gMouseMoved)(int x, int y);
static void (*gMouseDragged)(int x, int y);
static void (*gMouseFunc)(int button, int state, int x, int y);
static int gLastMousePositionX, gLastMousePositionY; // For glutGet
static unsigned int lastWidth = 400, lastHeight = 400; // For glutGet
static unsigned int winPosX = 40, winPosY = 40; // For glutGet

static unsigned int gContextInitMode = GLUT_RGBA | GLUT_DOUBLE | GLUT_DEPTH;
static void (*gIdle)(void);
static char updatePending = 1;
char gRunning = 1;
